...
#
# Parameters:
# [*ensure*] : If 'present' checks the existence of the output file (and
# downloads it if needed), if 'absent' deletes the output file, if not set
# redownload the artifact
...
# If repository is not set, its value is derived from the version contained in
# *gav*.  If the *gav* version is a SNAPSHOT then the repository will be set to
# 'snapshots', otherwise it will be 'releases'.
# If ensure is set to 'present' the resource checks the existence of the file
# and download the artifact if needed.
